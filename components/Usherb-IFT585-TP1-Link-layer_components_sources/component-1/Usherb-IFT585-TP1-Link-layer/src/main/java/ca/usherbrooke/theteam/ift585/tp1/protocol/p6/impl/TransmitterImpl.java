...
/**
 * Class that emulates the transmission of trames using the Selective Rejection algorithm.
 * @author Jules
 */
public class TransmitterImpl extends Transmitter {
	...
	public void run() {
		...
		while (currentSeq < getTramesHashMap().size()) {
			...
			// Check if NAK seq is set
			if (getController().getNakSeq() != -1) {

				// Transmit a NAK
				transmit(new Trame(getController(), getTramesHashMap().get(getController().getNakSeq()), TrameKind.Data, getController().getNakSeq()));

				// Clear NAK seq
				getController().setNakSeq(-1);
